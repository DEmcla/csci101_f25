...
- For short answer questions, show your work and explain your reasoning
...
**Instructions:** Circle T for True or F for False. Each question is worth 1.5 points.

1. **T / F** - In an Entity Relationship Diagram (ERD), a rectangle represents an entity and an oval represents an attribute.

2. **T / F** - A primary key can be composed of multiple columns working together (a composite key).

3. **T / F** - In crow's feet notation, a "crow's foot" symbol indicates the "many" side of a relationship.

4. **T / F** - First Normal Form (1NF) requires eliminating transitive dependencies.

5. **T / F** - A functional dependency X → Y means "if you know X, you can determine Y."

6. **T / F** - Bridge tables (junction tables) are used to resolve one-to-one relationships.

7. **T / F** - A partial dependency occurs when a non-key attribute depends on only part of a composite primary key.

8. **T / F** - Denormalization means intentionally adding redundancy back into a database, often for performance reasons.

9. **T / F** - Second Normal Form (2NF) requires that the table must first be in 1NF and have no partial dependencies.

10. **T / F** - A foreign key in one table references the primary key of another table, creating a relationship between them.

11. **T / F** - In a many-to-many relationship, you can directly connect two tables without a bridge entity.

12. **T / F** - Third Normal Form (3NF) is considered the industry standard for most database applications.

13. **T / F** - Cardinality refers to the number of instances of one entity that can be associated with instances of another entity.

14. **T / F** - BCNF, 4NF, and 5NF are more commonly used in practice than 3NF.

15. **T / F** - Update anomalies, insertion anomalies, and deletion anomalies are problems that normalization helps prevent.

---

## PART B: MULTIPLE CHOICE (45 points total)

**Instructions:** Circle the BEST answer for each question. Each question is worth 2.5 points.
...
|---------|---------------|-----------------|
| Part A (T/F) | _____ | 22.5 |
| Part B (Multiple Choice) | _____ | 45 |
| Part C (Short Answer) | _____ | 32.5 |
| **TOTAL** | **_____** | **100** |
